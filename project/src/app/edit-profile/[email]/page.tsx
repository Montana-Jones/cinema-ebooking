...
export default function EditProfile({
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === "homeAddress")
      setIsHomeAddressValid(validateAddress(newValue));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAttempts(0);
      setCustomer((prev) =>
        prev ? { ...prev, password: newPassword || prev.password } : prev
      );
      setOldPasswordInput("");
      setNewPassword("");
      setConfirmPassword("");
                ...
                {customer.paymentInfo.map((p, index) => (
                  <li key={index} className="py-2 text-sm">
                    ...
                    <p className="text-gray-500">
                      Experation Date: {p.expirationDate}
                    </p>
                    <p className="text-gray-500">
                      Card Holder Name: {p.cardHolder}
                    </p>
                  </li>
                ))}
              </ul>
                  ...
                  return (
                    <div key={index} className="border p-4 rounded-lg mb-3">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleCardChange(index, "cardHolder", e.target.value)
                        }
                        className={`w-full border rounded-lg px-3 py-2 mb-1 ${
                          errors.cardHolder
                            ? "border-red-500"
          ...
          {verifiedOldPassword && (
            <div className="border p-4 rounded-md  mt-4">
              <p className="font-medium mb-2">
                Set a new password(should be at least 8 characters):
              </p>
              <input
                type="password"
